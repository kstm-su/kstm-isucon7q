...
func init() {
	...
	exec.Command("bash", "/home/isucon/isubata/webapp/go/init.sh")
}
...
func main() {
	...
	e.POST("/imagesync/:file_name", syncImage)

    e.Start(":5000")
/*
    os.Remove("/tmp/isubata.sock")
    l, err := net.Listen("unix", "/tmp/isubata.sock")
    if err != nil {
      e.Logger.Fatal(err)
    }
    e.Listener = l
